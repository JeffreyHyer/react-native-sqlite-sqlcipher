...
SQLitePlugin.prototype.attach = function (
  dbNameToAttach,
  dbAlias,
  success,
  error
  ...
  if (this.dbname in this.openDBs) {
    if (txLocks[this.dbname] && txLocks[this.dbname].inProgress) {
      ...
      error(
        newSQLError(
          'database cannot be attached while a transaction is in progress'
        )
      );
      return;
    }
    ...
    return function (response) {
      if (!txFailure) {
        try {
          ...
          plugin.log(
            'warning - exception while invoking a callback: ' + errorMsg
          );
        }
